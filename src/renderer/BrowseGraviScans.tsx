...
export function BrowseGraviScans() {
  ...
  const [boxProgress, setBoxProgress] = useState<{ totalImages: number; completedImages: number; failedImages: number } | null>(null);
  const [scanActive, setScanActive] = useState(false);

  // Wave selector per experiment: expId -> selected wave number (default 0)
  const [selectedWaves, setSelectedWaves] = useState<Record<string, number | 'all'>>({});
  ...
  useEffect(() => {
    const cleanupBloom = window.electron.graviscan.onUploadProgress((progress) => {
      setUploadProgress({ total: progress.total, completed: progress.completed, failed: progress.failed });
    });
    const cleanupBox = window.electron.graviscan.onBoxBackupProgress((progress) => {
      setBoxProgress({ totalImages: progress.totalImages, completedImages: progress.completedImages, failedImages: progress.failedImages });
    });
    return () => { cleanupBloom(); cleanupBox(); };
  }, []);
  ...
  const handleUploadAll = useCallback(async () => {
    ...
    try {
      ...
      setUploadMessage(`Box backup failed: ${err instanceof Error ? err.message : 'Unknown error'}`);
    } finally {
      setUploading(false);
      setUploadProgress(null);
      setBoxProgress(null);
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        >
          {uploading ? 'Backing up...' : scanActive ? 'Scan in progress...' : 'Backup to Box'}
        </button>
      </div>

      {scanActive && (
        <div className="mb-4 p-3 rounded-lg border bg-amber-50 border-amber-200">
          <p className="text-sm text-amber-700">Please wait, still saving scans...</p>
        </div>
      )}
        ...
        <div className="mb-4 p-4 bg-white rounded-lg shadow-sm border border-gray-200">
          {uploadProgress && (
            <div className="mb-2">
              <div className="flex justify-between text-xs font-medium text-gray-600 mb-1">
                <span>Bloom</span>
                <span>{uploadProgress.completed}/{uploadProgress.total}{uploadProgress.failed > 0 ? ` (${uploadProgress.failed} failed)` : ''}</span>
              </div>
          ...
          {boxProgress && (
            <div>
              <div className="flex justify-between text-xs font-medium text-gray-600 mb-1">
                <span>Box</span>
                <span>{boxProgress.completedImages}/{boxProgress.totalImages}{boxProgress.failedImages > 0 ? ` (${boxProgress.failedImages} failed)` : ''}</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-3">
                <div
                  ...
                  style={{ width: `${boxProgress.totalImages > 0 ? Math.round((boxProgress.completedImages / boxProgress.totalImages) * 100) : 0}%` }}
                />
              </div>
            </div>
          )}
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {experiments.map((exp) => {
            ...
            const uniqueWaves = [...new Set(exp.scans.map((s) => s.wave_number))].sort((a, b) => a - b);
                ...
                <div className="flex">
                  ...
                  <div className="flex-1 p-4 border-r border-gray-100">
                    ...
                    <div className="mt-3 flex gap-2">
                      ...
                      <button
                        onClick={async () => {
                          const waveNum = selectedWave === 'all' ? undefined : selectedWave;
                          ...
                          const waveLabel = waveNum !== undefined ? ` (Wave ${waveNum})` : '';
                          if (result.copied > 0) {
                            alert(`Downloaded ${result.copied} of ${result.total} images${waveLabel}.${result.errors?.length ? `\n${result.errors.length} files could not be copied.` : ''}`);
                          } else if (result.total === 0) {
                            alert(`No image files found to download${waveLabel}.`);
                          } else {
                        ...
                        title={selectedWave === 'all' ? 'Download all waves' : `Download Wave ${selectedWave}`}
                      >
                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" />
                        </svg>
                        Download {selectedWave === 'all' ? 'All' : `Wave ${selectedWave}`}
                      </button>
                    </div>
                  ...
                  {(() => {
                    const bloomCount = allImages.filter((img) => img.status === 'uploaded').length;
                    const boxCount = allImages.filter((img) => img.box_status === 'uploaded').length;
                    const bloomPct = totalImages > 0 ? Math.round((bloomCount / totalImages) * 100) : 0;
                    const boxPct = totalImages > 0 ? Math.round((boxCount / totalImages) * 100) : 0;
                    const allBloom = totalImages > 0 && bloomCount === totalImages;
                      ...
                      <div className="w-44 flex-shrink-0 flex flex-col items-center justify-center p-4 border-r border-gray-100 gap-2">
                        ...
                        <div className="w-full">
                          <div className="flex items-center gap-1.5 mb-1">
                            <span className={`text-xs font-medium ${allBloom ? 'text-green-600' : 'text-gray-500'}`}>
                        ...
                        <div className="w-full">
                          <div className="flex items-center gap-1.5 mb-1">
                            <span className={`text-xs font-medium ${allBox ? 'text-green-600' : 'text-gray-500'}`}>
