...
async function drainPendingWritesAndSettle(
  ...
  console.log(`[GraviScan] DB writes drained after ~${waited / 1000}s, settling for ${SETTLE_DELAY_MS / 1000}s...`);
...
export interface UseScanSessionReturn {
  ...
  setScanImageUris: React.Dispatch<React.SetStateAction<Record<string, Record<string, string>>>>;
  scanningPlateIndex: Record<string, string>;
  setScanningPlateIndex: React.Dispatch<React.SetStateAction<Record<string, string>>>;
...
export function useScanSession({
  ...
  const {
    autoUploadStatus, setAutoUploadStatus,
    autoUploadMessage, setAutoUploadMessage,
  ...
  const [pendingJobs, setPendingJobs] = useState<Map<string, ScanJobInfo>>(new Map());
    ...
    const cleanupCycleComplete = window.electron.graviscan.onCycleComplete?.((data) => {
      ...
      if (pending.length > 0) {
        Promise.allSettled(pending).then(() => {
          pendingDbWritesRef.current = pendingDbWritesRef.current.filter(p => !pending.includes(p));
        });
      }
  ...
  useEffect(() => {
    const cleanup = window.electron.graviscan.onBoxBackupProgress?.((progress) => {
      setBoxBackupProgress({
