...
const phenotyperSchema = z.object({
  name: z.string().min(1, 'Name is required').max(255, 'Name is too long'),
  email: z.string().email('Must be a valid email address'),
});

type PhenotyperFormData = z.infer<typeof phenotyperSchema>;

interface PhenotyperFormProps {
  onSuccess: () => void;
}

export function PhenotyperForm({ onSuccess }: PhenotyperFormProps) {
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<PhenotyperFormData>({
    resolver: zodResolver(phenotyperSchema),
  });

  const onSubmit = async (data: PhenotyperFormData) => {
    setIsSubmitting(true);
    setSubmitError(null);

    try {
      // Data is validated by Zod schema before reaching here, so fields are guaranteed to be present
      const result = await window.electron.database.phenotypers.create({
        name: data.name,
        email: data.email,
      });
      ...
      if (!result.success) {
        ...
        const errorMessage = result.error || 'Failed to create phenotyper';
        const lowerError = errorMessage.toLowerCase();
        if (
          lowerError.includes('unique constraint') ||
          lowerError.includes('unique')
        ) {
          setSubmitError('A phenotyper with this email already exists');
        } else {
      ...
      console.error('Error creating phenotyper:', error);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
      ...
      >
        {isSubmitting ? 'Adding...' : 'Add new phenotyper'}
      </button>
    </form>
  );
}