...
function PlateCell({
  ...
  if (status === 'complete' && imageUri) {
    return (
      <div
        ...
        title={`Plate ${plate.plateIndex}${plate.plantBarcode ? ` — ${plate.plantBarcode}` : ''} (scanned)`}
      >
        <img
          src={imageUri}
          alt={`Scan plate ${plate.plateIndex}`}
        ...
        <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/70 to-transparent px-2 py-1.5">
          <div className="flex items-center justify-between">
            <span className="font-mono text-xs font-bold text-white">{plate.plateIndex}</span>
  ...
  if (status === 'scanning') {
    return (
      <div
        ...
        title={`Plate ${plate.plateIndex} — Scanning...`}
      >
        {/* Scanning animation */}
        ...
        <span className="font-mono text-xs text-blue-400 mt-0.5">{plate.plateIndex}</span>
        {plate.plantBarcode && (
  ...
  return (
    <div
      ...
      title={`Plate ${plate.plateIndex}${plate.plantBarcode ? ` — ${plate.plantBarcode}` : ''}${!plate.selected ? ' (not selected)' : ''}`}
    >
      {/* Scanner bed icon */}
