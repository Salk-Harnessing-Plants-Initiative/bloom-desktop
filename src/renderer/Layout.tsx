...
/** Home — rendered first, alone, in every mode */
const homeLink = {
  to: '/',
  label: 'Home',
  icon: (
...
/**
 * Shared "Setup" links visible in every mode — rendered after each mode's
 * Daily Workflow links, not first (see the `links` composition below).
 * Experiments is intentionally NOT in this array: it must be appended last,
 * after each mode's own Setup addition (Accessions/Metadata), to produce
 * that mode's target order.
 */
const setupLinks = [
  {
    to: '/scientists',
...
/**
 * Experiments — shared, but appended last in every mode's composition
 * (after that mode's own Setup addition), not part of `setupLinks`. See
 * `setupLinks`'s doc comment for why.
 */
...
/** Daily Workflow links (cylinderscan mode only) */
const captureLinks = [
  {
    to: '/camera-settings',
...
/**
 * Accessions — cylinderscan Setup addition, appended after `setupLinks`,
 * before `experimentsLink`.
 */
const accessionsLink = {
  to: '/accessions',
  label: 'Accessions',
  icon: (
    <svg
...
/**
 * Metadata — graviscan Setup addition, appended after `setupLinks`, before
 * `experimentsLink`.
 */
const metadataLink = {
  to: '/metadata',
