...
export function GraviScan() {
  ...
  const pendingJobsTemplateRef = useRef<
    Map<
      string,
      {
        scannerId: string;
  ...
  const [intervalCountdown, setIntervalCountdown] = useState<number | null>(
    null
  );
  const intervalCountdownRef = useRef<ReturnType<typeof setInterval> | null>(
    null
  );
        ...
        if (sessionIdRef.current) {
          ...
          (async () => {
            try {
              ...
              console.log(
                '[GraviScan] Waiting 45s for scan records and path renames to finalize...'
              );
              ...
              while (pendingDbWritesRef.current.length > 0) {
                ...
                console.log(
                  `[GraviScan] Draining ${pending.length} pending DB writes...`
                );
      ...
      try {
        ...
        if (!sessionFinishedWhileAway) {
          ...
          if (status.scanStartedAt) {
            ...
            setElapsedSeconds(
              Math.floor((Date.now() - status.scanStartedAt) / 1000)
            );
            if (elapsedTimerRef.current) clearInterval(elapsedTimerRef.current);
            elapsedTimerRef.current = setInterval(() => {
              if (scanStartedAtMsRef.current) {
                setElapsedSeconds(
                  Math.floor((Date.now() - scanStartedAtMsRef.current) / 1000)
                );
              }
            }, 1000);
          }
  ...
  async function handleStartScan() {
    ...
    try {
      ...
      const newPendingPlates = new Map<
        string,
        {
          scannerId: string;
          plateIndex: string;
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <span className="text-green-700 text-sm font-medium">
                {configValidationMessage} - {matchedScanners.length} scanner
                {matchedScanners.length > 1 ? 's' : ''} configured
              </span>
            </div>
          </div>
        </div>
            ...
            <div className="flex-1">
              ...
              {missingScanners.length > 0 && (
                <ul className="mt-1 text-amber-600 text-xs">
                  {missingScanners.map((s) => (
                    <li key={s.id}>
                      • {s.name} (was on port {s.usb_port}) - not detected
                    </li>
                  ))}
                </ul>
              )}
              {newScanners.length > 0 && (
                <ul className="mt-1 text-amber-600 text-xs">
                  {newScanners.map((s) => (
                    <li key={s.usb_port}>
                      • New scanner on port {s.usb_port} - {s.name}
                    </li>
                  ))}
                </ul>
              )}
                                    ...
                                    {!isInProgress &&
                                      ...
                                      result.scanTimeMs && (
                                        <p className="text-xs text-green-600 mt-0.5">
                                          ...
                                          {(result.scanTimeMs / 1000).toFixed(
                                            1
                                          )}
                                          s
                                        </p>
      ...
      >
            ...
            {assignedScannerIds.length === 0 && (
              <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg text-center">
                <p className="text-sm text-gray-500">
                  No scanners assigned. Configure scanners above to assign
                  plates.
                </p>
              </div>
            )}
        ...
        {sessionValidated &&
          ...
          !isScanning &&
          scannerStates.every((s) => s.state === 'idle') && (
            <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex items-center">
                <svg
                ...
                <span className="text-sm text-green-700">
                  {scannerStates.length} scanner
                  {scannerStates.length > 1 ? 's' : ''} connected and ready
                </span>
              </div>
            </div>
          )}
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setScanIntervalMinutes(
                        Math.max(
                          MIN_SCAN_INTERVAL_MINUTES,
                          parseInt(e.target.value) || MIN_SCAN_INTERVAL_MINUTES
                        )
                      )
                    }
                    disabled={isScanning}
            ...
            <div className="mt-3 p-3 bg-indigo-50 border border-indigo-200 rounded-lg space-y-1">
              ...
              {overtimeMs !== null && (
                <div className="mt-2 p-2 bg-amber-50 border border-amber-300 rounded text-sm text-amber-800">
                  Scans need additional time beyond the set duration (+
                  {Math.floor(overtimeMs / 60000)}m{' '}
                  {Math.floor((overtimeMs % 60000) / 1000)}s)
                </div>
              )}
            </div>
          )}
