...
export function GraviScan() {
      ...
      for (const assignment of assignedBarcodes) {
        try {
          const result = await window.electron.database.graviscans.checkBarcodeUniqueInWave({
            ...
            plant_barcode: assignment.plantBarcode!,
          });
  ...
  useEffect(() => {
        ...
        const dbWritePromise = (async () => {
          try {
            const graviscanResult = await window.electron.database.graviscans.create({
              ...
              plant_barcode: jobInfo.plantBarcode || null,
    ...
    const cleanupGridComplete = window.electron.graviscan.onGridComplete(async (data) => {
      ...
      if (pendingDbWritesRef.current.length > 0) {
        await Promise.allSettled(pendingDbWritesRef.current);
      ...
      if (recordIds && recordIds.length > 0) {
        window.electron.database.graviscans
          ...
          .catch((err) => console.error('[GraviScan] Failed to update grid timestamps:', err));
    ...
    const cleanupIntervalComplete = window.electron.graviscan.onIntervalComplete?.((data) => {
      ...
      // Mark session as complete in DB
      if (sessionIdRef.current) {
        window.electron.database.graviscanSessions
          .complete({ session_id: sessionIdRef.current })
          .catch((err) => console.error('[GraviScan] Failed to complete session:', err));
          ...
          if (jobMeta && !jobMeta.dbRecorded && status.experimentId && status.phenotyperId) {
            try {
              const graviscanResult = await window.electron.database.graviscans.create({
                ...
                plant_barcode: jobMeta.plantBarcode || null,
    ...
    try {
      ...
      const scannerConfigs = enabledScanners.map((scanner, scannerIdx) => {
        ...
        const plates = selectedPlatesForScanner.map((plate) => {
          const filename = `${sanitizedExpName}_st_${timestamp}_cy1_S${scannerIdx + 1}_${plate.plateIndex}.tif`;
          ...
          return {
            ...
            plant_barcode: plate.plantBarcode || null,
          };
        });
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <ScanPreview
          ...
          onImageClick={(imageUri, plateIndex) =>
            setLightboxImage({ src: imageUri, caption: `Plate ${plateIndex}` })
          }
        />
      </div>
