...
export function registerGraviscanHandlers(
    ...
    async (
      ...
      params: {
        scanners: Array<{
          scannerId: string;
          saneName: string;
          plates: (PlateConfig & { plant_barcode?: string | null })[];
        }>;
        ...
        for (const s of params.scanners) {
          for (const plate of s.plates) {
            ...
            jobs[key] = {
              ...
              plantBarcode: plate.plant_barcode ?? null,
              transplantDate: null,
              customNote: null,
  ...
  ipcMain.handle('graviscan:read-scan-image', async (_event, filePath: string, options?: { full?: boolean }) => {
    try {
      // Resolve file path — try alternate extensions if DB path doesn't match disk
      if (!fs.existsSync(filePath)) {
        const ext = path.extname(filePath);
        const base = ext ? filePath.slice(0, -ext.length) : filePath;
        const altExts = ['.tif', '.tiff', '.jpg', '.jpeg', '.png'].filter((e) => e !== ext);
        let found = false;
        for (const altExt of altExts) {
          if (fs.existsSync(base + altExt)) {
            filePath = base + altExt;
            found = true;
            break;
          }
        }
        // Fallback 2: Try finding renamed file with _et_ inserted after _st_ timestamp
        if (!found) {
          ...
          if (etMatch && fs.existsSync(dir)) {
            ...
            if (candidates.length === 1) {
              ...
              found = true;
              console.log(`[read-scan-image] Resolved via _et_ fallback: ${basename} -> ${candidates[0]}`);
            }
          }
        }
        if (!found) {
        ...
        console.log(`[read-scan-image] Resolved: ${base}${ext} -> ${filePath}`);
      }
      const buffer = fs.readFileSync(filePath);
      const lowerPath = filePath.toLowerCase();
      const isTiff = lowerPath.endsWith('.tif') || lowerPath.endsWith('.tiff');

      // Chromium cannot render TIFF — convert to JPEG for preview
      if (isTiff) {
      ...
      const ext = lowerPath.endsWith('.png') ? 'png' : 'jpeg';
      return {
        success: true,
   ...
   * Sends progress events to the renderer via 'graviscan:upload-progress'.
   */
  ipcMain.handle('graviscan:upload-all-scans', async () => {
    try {
      console.log('[GraviScan:UPLOAD] Starting upload for all pending/failed scans');

      const mainWindow = getMainWindow?.();

      const result = await uploadAllPendingScans(db, (progress) => {
        mainWindow?.webContents.send('graviscan:upload-progress', progress);
      });

      console.log('[GraviScan:UPLOAD] Complete:', result);
      return result;
    } catch (error) {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        // Query all images for this experiment
        ...
        // Determine if continuous mode (any scan has cycle_number)
        const isContinuous = scans.some((s) => s.cycle_number !== null);

        // Collect all image files to copy — use original filenames
        const filesToCopy: { src: string; dest: string }[] = [];
        for (const scan of scans) {
          const scannerName = scan.scanner?.display_name || scan.scanner?.name?.replace(/[^a-zA-Z0-9_-]/g, '_') || `Scanner_${scan.scanner_id.slice(0, 8)}`;

          for (const img of scan.images) {
            // Resolve actual file path — DB may have stale path
            let srcPath = img.path;
            if (!fs.existsSync(srcPath)) {
              // Fallback 1: Try alternate extensions (.jpg ↔ .tif ↔ .png)
              ...
              if (!fs.existsSync(srcPath)) {
                ...
                const match = basename.match(/^(.+_st_\d{8}T\d{6})(_cy.+)$/);
                if (match && fs.existsSync(dir)) {
                  ...
                  if (candidates.length === 1) {
                    srcPath = path.join(dir, candidates[0]);
                  }
                }
              }
            }

            // Skip images whose source file cannot be found
            if (!fs.existsSync(srcPath)) {
              continue;
            }
            ...
            if (isContinuous && scan.cycle_number !== null) {
              const waveDir = `Wave_${String(scan.cycle_number).padStart(2, '0')}`;
              destPath = path.join(targetDir, params.experimentName, scannerName, waveDir, originalFilename);
            } else {
              destPath = path.join(targetDir, params.experimentName, scannerName, originalFilename);
            }
        ...
        console.log(`[GraviScan:DOWNLOAD] Copied ${copied}/${filesToCopy.length} images to ${targetDir}`);
