...
A separate test targets the "hardware-module diagnostic line" scenario the fix's expanded scope exists for: reuse the same fake-stdout harness, but make thread B call a hardware-module print path (e.g. a helper wrapping one of `camera_mock.py`'s routed `write_line()` calls) instead of a synthetic payload — proving the _actual_ routed call sites, not just the four original helpers, are covered by the same serialization.
