...
- [x] 1.3 Write a failing test asserting write-then-rename *order*, not just end-state: mock `os.replace` with a `side_effect` that itself asserts, at call time, that `final_path` does NOT yet exist and the temp path DOES exist with the expected content — this proves the implementation can't satisfy the test via a direct-write-then-no-op-temp-file shortcut.
...
- [x] 1.3b Write a failing test for a rename failure *after* a successful save: mock `os.replace` to raise (e.g. `PermissionError`) after the temp file was genuinely written; assert the exception propagates (not caught/swallowed by the helper) and no file exists at `final_path`.
