...
- [x] 2.7 **NOT EXECUTED ON HARDWARE — covered by unit test plus a static trace, which is a more honest status than either "pass" or "not executed".** The first run terminated at 2.4 before reaching session-cancel. A second attempt (2026-09-16) reproduced the wedge but failed on a spec-side bug of my own: it started the session over IPC, so the renderer never entered `isScanning` and the scan page's Cancel button was never rendered at all (`cancelCount: 0`) — the same architectural limitation as the first run, which also drove everything by IPC and only ever clicked the globally-rendered banner buttons.

      The substantive question behind this item — whether the main process still emits `cancelled` when the session's only scanner has been auto-paused and removed from `this.subprocesses` — is answered without hardware: `cancelAll()` emits `'cancelled'` unconditionally (`scan-coordinator.ts`, outside any guard on the subprocess map), `wiring.ts` forwards it unconditionally to the renderer, and `useWedgeEvents`'s `clearAll` is subscribed to it. The renderer half is unit-tested at `tests/unit/hooks/useWedgeEvents.test.ts` ("resets both counters to 0 when the session ends (interval-complete/cancelled)"). What remains unverified is only the physical button click and the DOM node disappearing. Executing it properly would mean driving the whole UI flow — experiment and phenotyper selection, plate assignment — which is a much larger spec and runs into #367's config friction. User decision (2026-09-16): do not spend a second power cycle on it.
...
> **Archive note.** Four items are marked complete-by-disposition rather than
> executed-and-passed, each deferred to a tracked issue: **2.3** → #369
> (needs a second V600), **2.4** → #182 (reproduced defect, assigned),
> **2.7** → covered by unit test plus a static trace (see the item), and
> **3.3** records that #279 deliberately stays open. Nothing here claims a
> pass that did not happen; #279 remains a Tier 2 cutover hard-block, as
> does #364.
...
- [x] 3.3 **#279 stays OPEN — recorded, not closed.** 2.3 is blocked on scanner count, 2.4 fails, 2.7 was not reached, and 2.2's correlation assertion is unverified. This supersedes the original instruction to close it. #279 remains a Tier 2 hard-block.
