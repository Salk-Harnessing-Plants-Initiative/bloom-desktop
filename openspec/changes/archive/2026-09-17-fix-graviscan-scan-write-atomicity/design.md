...
- Leftover `.tmp-*` files from a **SIGKILL** are not cleaned up (accepted, Decision 1) — a killed process runs no handler — but are now excluded from the app's own file listing, so they can't be mistaken for real data within the app. **Handled** failures (save error, rename error) now do clean up before propagating: review noted that `_sane_scan()` retries a failed plate up to `MAX_RETRIES` times with a fresh temp name each attempt, so leaking on a _handled_ error would strand up to five full-resolution TIFFs per plate — invisible, since the app hides the prefix, and unbounded across a multi-day run. Review round 4 checked the two in-repo precedents originally cited for this and found the argument was backwards: neither actually cleans up its _own_ temp on a handled failure — `cylinderscan/scan-metadata-json.ts` deletes a _stale_ tmp before writing and has no failure path, and `database-handlers.ts` catches the error and records a reason without unlinking. The behaviour shipped here is correct and stricter than both; it simply does not have the precedent it claimed.
- These temp files are **dotfiles**, so they are hidden from `ls`, from shell globs, and from Python's `glob.glob('*.tif')`, as well as from the app's own file browser. "Distinguishable at a glance" therefore requires `ls -a`. Worth a line in the Tier 2 cutover runbook so a stray-file sweep is an operational step rather than a surprise.
- **Correction (review round 5):** an earlier version of this bullet claimed the dotfile naming "shields any downstream pipeline that globs the scans directory". That is **wrong**, and it was the stated basis for accepting the residual. `glob.glob` hides dotfiles; `pathlib.Path.glob`, `Path.rglob` and `os.listdir` do **not** — verified directly:

      glob.glob    : ['Exp_00.tif']
      pathlib.glob : ['.tmp-abc123-Exp_00.tif', 'Exp_00.tif']
      os.listdir   : ['.tmp-abc123-Exp_00.tif', 'Exp_00.tif']

  `pathlib` is the dominant idiom in modern Python and in the sleap-roots ecosystem this data feeds, so a truncated full-resolution TIFF **can** be picked up by an analysis pipeline as a valid timepoint — it has a valid `.tif` extension and carries the full final basename after the uuid. Nothing in the codebase ever deletes a `.tmp-` file, so an orphan is permanent as well as invisible in the app.

  The residual is still accepted here (only a SIGKILL can strand one, since every handled failure now cleans up), but it is accepted on honest grounds: the runbook sweep is the mitigation, not the naming. A `.tmp-` reaper at session start plus a free-space precondition would close it properly and is worth its own issue rather than another deferral in this document.
