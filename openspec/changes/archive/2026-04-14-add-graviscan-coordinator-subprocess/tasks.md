...
- [x] Write test: after cancelAll during interval wait, isScanning returns false (RED)
- [x] Fix: scanInterval sets state='idle' before emitting interval-complete (GREEN)

**B2: No timeout on Promise.all in scanOnce — hang risk**

- [x] Write test: per-row scan timeout triggers scan-error for non-responding subprocess (RED)
- [x] Fix: add SCAN_ROW_TIMEOUT_MS with per-row timeout that rejects hanging promises (GREEN)

**B3: scan_ended_at: null injected into forwarded events**

- [x] Write test: forwarded scan-complete events do not include scan_ended_at (RED)
- [x] Fix: remove scan_ended_at from forwarded events (only include in grid-complete) (GREEN)

**B4: Multi-scanner tests use real 5s stagger delay**

- [x] Refactor stagger/partial-failure/cancel tests to use vi.useFakeTimers (no behavioral change)

**B5: Flaky cancel-during-interval test**

- [x] Rewrite to use fake timers with deterministic advancement (no behavioral change)

**I1: Regex path rewriting applied to full path, not just basename**

- [x] Write test: output_path with date-like directory name doesn't get mangled (RED)
- [x] Fix: apply regex to path.basename only, rejoin with dirname (GREEN)

**I2: scanOnce doesn't check cancelled after Promise.all**

- [x] Write test: after cancel during active scan, no grid-complete emitted for remaining rows (RED)
- [x] Fix: check this.cancelled after Promise.all returns, skip file verification (GREEN)

## Task 8: Verify all fixes pass and no regressions

- [x] `npx tsc --noEmit` passes
- [x] `npm run test:unit` passes (all tests including new ones)
- [x] Multi-scanner tests complete in <2s (no real stagger delays)
