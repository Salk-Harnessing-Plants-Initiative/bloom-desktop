...
- Add `timeout-minutes` to the five jobs newly exposed to indefinite blocking by the queuing behavior above (`build-python`, `test-integration`, `test-e2e-dev`, `test-make`, `test-make-windows`) — without a `concurrency` block, a hung job only delayed its own push; with queuing, it could otherwise also block the _next_ queued `main` commit's CI from starting until GitHub's 6-hour default elapses. `build-python` is included because the other four all `needs: build-python`, so an unbounded hang there would defeat their own bounds. Values are set from actual observed job durations (see `design.md`), not guessed.
...
- Add `js-yaml` and `@types/js-yaml` as explicit `devDependencies` (`js-yaml` was already present transitively via `@salk-hpi/bloom-fs`/`eslint`; `@types/js-yaml` is new since `js-yaml` ships no bundled types) since the new test imports `js-yaml` directly — this requires regenerating `package-lock.json`, not just editing `package.json`, so `npm ci` in every CI job keeps working.
