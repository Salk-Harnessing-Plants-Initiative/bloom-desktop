...
## Real-CI-caught bug (fixed) — bulk `shutdown()` didn't clear `spawnInFlight`

CI on the opened PR (#357) failed the same real test on all three platforms (Linux, macOS, Windows): `graviscan-ipc.e2e.ts`'s "Reset All USB Connections marks rows starting, then settles back to a populated list", timing out waiting for any scanner to reach `ready` within 45s — not flaky, reproduced consistently including on retry.

Root cause, found by reading the actual CI logs (`gh api .../jobs/<id>/logs`, not just `gh pr checks`'s summary, which only shows the latest per-check status and hid the real failure): the IMPORTANT fix above (bulk `shutdown()` now calls `sub.removeAllListeners()` before force-killing) combined with a pre-existing gap — bulk `shutdown()` never cleared `this.spawnInFlight` — to reintroduce exactly the class of bug this whole change exists to fix, in a new location:

1. "Detect Scanners" (the E2E test's own first step) calls `addScanner()` (via the `graviscan:save-scanners-db` IPC handler in `register-handlers.ts`), which starts a spawn for a scanner and holds a `spawnInFlight` entry for it while `sub.spawn()` is still pending.
2. "Reset All USB Connections" (`resetUsb()` in `scanner-handlers.ts`) calls `coordinator.shutdown()` (the bulk method) first, while that scanner is still mid-spawn. Bulk `shutdown()`'s new `removeAllListeners()` strips the listeners `sub.spawn()`'s own promise executor depends on to ever settle via the normal ready/exit path — so that orphaned `addScanner()`-triggered `spawnSingleScanner()` call can now **only** settle via its own `SPAWN_READY_TIMEOUT_MS` (45s) bound, exactly like the accepted residual in Decision 1's `stopScanner()` fix.
3. Bulk `shutdown()` never cleared `this.spawnInFlight` for that scanner (unlike `stopScanner()`, which does this as its very first step). So `resetUsb()`'s own subsequent `coordinator.initialize(...)` call for the same scanner found the stale, now-45s-bound entry still in `spawnInFlight` and was handed that orphaned promise instead of spawning fresh — stalling the entire reset for the full 45 seconds instead of respawning within the ~10s the test actually budgets for it.

**Fix**: bulk `shutdown()` now clears `this.spawnInFlight` for each scanner it tears down, before stripping listeners — matching `stopScanner()`'s own convention exactly. `killAll()` (the app-quit fallback) got the same fix defensively (`this.spawnInFlight.clear()`), even though it doesn't call `removeAllListeners()` and so isn't known to be reachable by this exact race today.

`doInitialize()`'s stale-subprocess cleanup loop was checked for the same gap and found NOT to need this fix: it never calls `removeAllListeners()`, so an orphaned spawn attempt torn down by that loop still settles quickly via its own listeners (the existing identity-guard from the first BLOCKING fix above already prevents that settlement from falsely reporting or clobbering a newer instance) — adding `removeAllListeners()` there for consistency would have reintroduced the same class of bug this section just fixed, for no correctness benefit, so it was deliberately left as-is.

A new regression test reproduces the exact real trigger sequence (`addScanner()` for the still-mid-spawn scanner → bulk `shutdown()` → a fresh `initialize()` for the same scanner) and was verified to fail red against the pre-fix code the same way as every other fix in this document. This is also a concrete instance of the project's own standing lesson that real E2E runs against a live Electron app catch classes of bugs mocked unit tests structurally cannot see (here: real `EventEmitter` listener semantics across a forced-kill path) — every unit test for the bulk-shutdown fix used a mocked `ScannerSubprocess` and could not have caught this on its own.
