...
1. **Snapshot** (`snapshotDescendants(pid)`), called immediately after capturing `pid`, inside its own try/catch, before `electronApp.close()` — while the parent-child relationships are still queryable:
   ...
   - Build a parent→children adjacency list from the full table, then breadth-first-walk from `pid` to collect every **transitive** descendant (children, grandchildren, ...), not just direct children, recording `{pid, name}` for each.
...
- **transitive descendants**: the first child itself spawns 2 further children of its own (modeling the onefile-bootloader nesting from Round 4) — both snapshot and kill are verified to reach them, not just the direct children
