...
1. Fix `closeElectronApp()` in `tests/e2e/helpers/electron-cleanup.ts` to snapshot and force-kill the **full transitive descendant tree** of the main Electron PID (Electron Helper, the Python `bloom-hardware` subprocess, and any of their own children — not just direct children, since `bloom-hardware`'s PyInstaller onefile bootloader can itself introduce an extra level of nesting), not just the main PID itself — see `design.md` Decision 1 for the snapshot-before/kill-after ordering, the exception isolation between the new snapshot step and the existing main-process teardown, the cross-platform kill mechanism, and the PID-recycle guard.
