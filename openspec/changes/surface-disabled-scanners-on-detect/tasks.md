...
- [ ] 1.1 Add to `tests/unit/graviscan/scanner-handlers.test.ts`:
...
- [ ] 1.2 Add to `tests/unit/hooks/useScannerConfig.test.ts`:
...
- [ ] 1.3 Run failing tests:
  ```bash
  npm run test:unit -- tests/unit/graviscan/scanner-handlers.test.ts tests/unit/hooks/useScannerConfig.test.ts --run
  ```
  Confirm new tests fail for behavior reasons, not type/compile reasons.

**Check gate (after 1.3):** New tests fail. Existing tests still pass.

## 2. Implementation — green phase

- [ ] 2.1 Add `enabled?: boolean` to `DetectedScanner` interface in `src/types/graviscan.ts`. Document that absent means "treat as true (newly-discovered scanner default)" and present means "this came from a DB row with that enabled state."

- [ ] 2.2 In `src/main/graviscan/scanner-handlers.ts`:
...
- [ ] 2.3 In `src/renderer/hooks/useScannerConfig.ts`, update `buildAssignmentsFromDetection`:
...
- [ ] 2.4 Run the test suite:
  ...
  All previously-failing tests SHALL now pass. No existing tests SHALL regress.

- [ ] 2.5 Run the full unit suite + tsc + lint + prettier to confirm no collateral damage:
  ```bash
  npm run test:unit
  npx tsc --noEmit
  npm run lint
...
- [ ] 3.1 Restart dev server: `GRAVISCAN_MOCK=true npm run start`
- [ ] 3.2 Confirm Scanner Configuration page shows BOTH Scanner 1 (enabled, checked) and Scanner 2 (disabled from previous test, unchecked).
- [ ] 3.3 Click Scanner 2's checkbox to re-enable it.
- [ ] 3.4 Click Save Configuration.
- [ ] 3.5 Confirm green success banner appears with "2 scanners saved · 2-Grid · 1200 DPI".
- [ ] 3.6 Verify in DB:
  ```bash
  sqlite3 ~/.bloom/dev.db "SELECT name, enabled FROM GraviScanner;"
  ```
  Both rows SHALL have `enabled=1`.
- [ ] 3.7 Repeat the round-trip: uncheck Scanner 2, save, reload renderer, confirm Scanner 2 still appears (unchecked) and can be re-enabled.

## 4. Final verification

- [ ] 4.1 `npm run test:unit` passes
- [ ] 4.2 `npx tsc --noEmit` clean
- [ ] 4.3 `npm run lint` clean
- [ ] 4.4 `npx prettier --check` clean
- [ ] 4.5 `openspec validate surface-disabled-scanners-on-detect --strict` passes
