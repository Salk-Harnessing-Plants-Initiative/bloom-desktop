...
- [ ] 1.1 Update `tests/unit/pages/ScannerConfig.test.tsx`:
...
- [ ] 1.2 Update `tests/unit/hooks/useScannerConfig.test.ts` with:
...
- [ ] 1.3 Update `tests/unit/graviscan/scanner-handlers.test.ts`:
...
- [ ] 1.4 Update `tests/unit/pages/GraviScan.test.tsx`:
...
- [ ] 1.5 Update `tests/unit/hooks/useScanSession.test.ts` (REQUIRED — the previous "no edits needed" claim was wrong):
...
- [ ] 1.6 Create `tests/unit/graviscan/session-handlers-metadata.test.ts`:
...
- [ ] 2.1 In `src/renderer/hooks/useScannerConfig.ts`, populate `scannerAssignments` on detection. When `handleDetectScanners` and the `'no-config'` branch of `validateScannerConfig` finish loading scanners, map each `DetectedScanner` into a `ScannerAssignment` with `scannerId` set. The existing `matchDetectedToDb` path already does this for `'valid'` and `'mismatch'` branches — extend consistently.

- [ ] 2.2 Add a persisted unchecked-state map:
...
- [ ] 2.3 Rewrite the auto-save effect (lines ~477-559) to:
...
- [ ] 2.4 **BREAKING:** Remove `handleToggleScannerEnabled` from `useScannerConfig`'s return. Remove the `scannerStates` / `setScannerStates` parameters from `UseScannerConfigParams`. The hook no longer reads or writes `scannerStates`. Update the hook's return type and all internal references.

- [ ] 2.5 In `src/renderer/graviscan/ScannerConfig.tsx`:
...
- [ ] 2.6 In `src/renderer/graviscan/GraviScan.tsx`:
...
- [ ] 2.7.1 **Prerequisite for 2.7 read-site edits:** Plumb `scannerAssignmentsRef` through `UseScanSessionParams`:
...
- [ ] 2.7 **BREAKING (internal):** Remove `enabled: boolean` from `ScannerPanelState` in `src/types/graviscan.ts`. Update all readers and writers:
...
- [ ] 2.8 In `src/main/graviscan/scanner-handlers.ts`:
...
- [ ] 2.9 In `src/main/graviscan/scanner-handlers.ts`, add a new handler `disableMissingScanners(db, enabledIdentities)`:
...
- [ ] 2.10 In `src/main/graviscan/session-handlers.ts` line 196:
...
- [ ] 3.1 Create `tests/e2e/graviscan-scanner-config-save.e2e.ts`:
...
- [ ] 3.2 Add the new E2E file to `playwright.config.ts` if a glob doesn't pick it up automatically
- [ ] 3.3 Manual smoke: with dev server running and `GRAVISCAN_MOCK=true`, walk the flow — Scanner Config → Save → `sqlite3 ~/.bloom/dev.db "SELECT COUNT(*) FROM GraviScanner WHERE enabled=1"` should be ≥ 1 → Metadata loads without Prisma errors in the terminal.
- [ ] 3.4 Manual smoke: uncheck all scanners; confirm Save button disabled and helper text visible.
- [ ] 3.5 Manual smoke: rapid double-click Save — confirm only one round-trip in the terminal log and one success banner.
- [ ] 3.6 Manual smoke: uncheck one scanner, Cmd-R to reload renderer, assert the scanner is still unchecked on the re-rendered page.
- [ ] 3.7 Manual smoke: perform a scan, open the resulting scan directory, check the `*.metadata.json` file's `scanner_name` field — should be the scanner's `display_name` (or `name`), NOT a UUID.
...
- [ ] 4.1 File GitHub issue **#201**: "GraviScan orphan-row diagnostic for dev DBs". Query:
...
- [ ] 4.2 File GitHub issue **#202**: "Metadata page should read scanners from GraviScanner table, not re-run detection". Currently `Metadata.tsx:61` calls `detectScanners()` — couples metadata to physical scanner connectivity. Note that after this PR, `GraviScanner.enabled` reliably reflects user configuration, so the new issue becomes actionable with a simple `graviscans.list({ enabled: true })` IPC. Update proposal.md once assigned.

- [ ] 4.3 Comment on existing issue **#167**: Link this PR and note that aligning both renderer and main-process on `usb_port` as primary identity makes the future `@@unique(usb_port)` migration safe. Do NOT file a new issue for #201 — it duplicates #167.

- [ ] 4.4 Comment on existing issue **#159**: Link this PR and note that after merge, the fix-site shifts from `scannerStates.some(s => s.enabled)` to `scannerAssignments.some(a => a.scannerId !== null)`. Update #159 if it references the old path.

- [ ] 4.5 Comment on existing issue **#199**: Link this PR's inline strengthening of `ScannerConfig.test.tsx` and reference new issue **#200** (lint rule for bare `toHaveBeenCalled`). The broader audit remains open for the non-Scanner-Config test files.

- [ ] 4.6 File GitHub issue **#203**: "USB device-number instability during reconnect (follow-up to #182)". Even after this PR, there are edge cases where a physical scanner moves to a DIFFERENT `usb_port` (e.g., user moves the cable). The current PR documents this as a known limitation in the spec's requirement preamble. A future fix would require cross-session scanner identity tracking. Link to #182. Update proposal.md once assigned.

## 5. Final verification

- [ ] 5.1 `npm run test:unit` passes (new tests + all existing tests)
- [ ] 5.2 `npm run test:e2e -- graviscan-scanner-config-save` passes locally
- [ ] 5.3 `npx tsc --noEmit` clean
- [ ] 5.4 `npm run lint` clean
- [ ] 5.5 `npx prettier --check "**/*.{ts,tsx,md,json}"` clean
- [ ] 5.6 `openspec validate fix-scanner-config-save-flow --strict` passes
- [ ] 5.7 All "to be filed" placeholders in proposal.md have been updated with actual assigned issue numbers from task 4.x

## 6. Process improvements (to apply from now on)

- [ ] 6.1 Adopt rule for all new tests in this branch and future work: every test must assert either (a) call payload via `toHaveBeenCalledWith(...)` or access to `mock.calls`, OR (b) post-state (rendered UI, store state, IPC response). Bare `expect(fn).toHaveBeenCalled()` is no longer acceptable. Tracked in issue #200 (lint rule). Until #200 lands, enforce via review.
- [ ] 6.2 Adopt rule: before any OpenSpec review round is considered "complete," run the actual scenario end-to-end with the dev server + mock mode. Adversarial subagents read code, they do not click buttons. Review rounds that only read are worth half of their previous weight.
