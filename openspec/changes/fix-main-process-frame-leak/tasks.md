...
- [ ] 2.1 Write unit tests in `tests/unit/frame-forwarder.test.ts`:
...
- [ ] 2.2 Run tests — verify RED (frame-forwarder.ts does not exist yet)
- [ ] 2.3 Create `src/main/frame-forwarder.ts` with `createFrameForwarder(getSendFn)`:
...
- [ ] 2.4 Wire into `ensureCameraProcess()` in `main.ts`, replacing the current `cameraProcess.on('frame', ...)` handler
- [ ] 2.5 Run tests — verify GREEN
- [ ] 2.6 Run full TS test suite: `npx vitest run`
- [ ] 2.7 Commit: `fix: add frame-dropping gate to prevent IPC queue leak`
...
- [ ] 3.5 Commit any remaining fixes
