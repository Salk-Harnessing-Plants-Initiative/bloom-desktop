/**
 * Unit Tests: PhenotyperForm Component
 *
 * Tests for the PhenotyperForm component to ensure validation,
...
import { PhenotyperForm } from '../../../src/renderer/components/PhenotyperForm';

// Mock the window.electron.database.phenotypers.create API
...
beforeEach(() => {
  ...
  // Override the global window.electron.database.phenotypers.create
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const win = global.window as any;
  if (win && win.electron && win.electron.database) {
    win.electron.database.phenotypers.create = mockCreate;
  }
});

describe('PhenotyperForm', () => {
  it('should render all form fields', () => {
    const mockOnSuccess = vi.fn();
    render(<PhenotyperForm onSuccess={mockOnSuccess} />);

    // Verify labels and inputs are present
    expect(screen.getByLabelText('Name')).toBeInTheDocument();
    expect(screen.getByLabelText('Email')).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: /add new phenotyper/i })
    ).toBeInTheDocument();
  });
  ...
  it('should display duplicate email error message', async () => {
    ...
    mockCreate.mockResolvedValue({
      success: false,
      error: 'UNIQUE constraint failed: Phenotyper.email',
    });
    ...
    await waitFor(() => {
      expect(
        screen.getByText('A phenotyper with this email already exists')
      ).toBeInTheDocument();
    });
