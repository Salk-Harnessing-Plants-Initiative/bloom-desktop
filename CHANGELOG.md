...
- Layout sidebar/nav parity (closes [#328](https://github.com/Salk-Harnessing-Plants-Initiative/bloom-desktop/issues/328), [#337](https://github.com/Salk-Harnessing-Plants-Initiative/bloom-desktop/issues/337)) — picks up the cross-mode work Tier 4 deferred above, now that #289/#290 have merged
  - `Layout.tsx`'s shell background (`bg-gray-50` → `bg-stone-100`) and sidebar nav-link hover/active states (blue → lime/stone) now match the lime/stone/amber convention everywhere else, across both scan modes. Uses `lime-800`, not `lime-700`, specifically for WCAG AA contrast (`lime-700` on `bg-stone-200` computes to ≈3.98:1, below the 4.5:1 minimum; `lime-800` clears it at ≈5.64:1/≈6.49:1)
  - New `GraviScanWorkflowGuide` component gives GraviScan's Home screen the same "Daily Workflow" (Configure Scanner, Capture Scan, Browse GraviScans) / "Setup" (Scientists, Phenotypers, Metadata, Experiments) restructure CylinderScan got in Tier 4 via `CylinderScanWorkflowGuide`. `WorkflowSteps.tsx` and its `graviScanSteps` data are retired entirely (no remaining consumers)
