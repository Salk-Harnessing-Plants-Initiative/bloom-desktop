...
DEVICE="${1:-epkowa:interpreter:011:007}"
RESOLUTION=200
...
echo "[1/3] Connecting..."
START=$(date +%s)
SANE_DEBUG_EPKOWA=128 scanimage -d "$DEVICE" --x-resolution=200 --y-resolution=200 --format=tiff -x 10 -y 10 > /dev/null 2>"$OUT/connect.log"
if [ $? -eq 0 ]; then
...
START=$(date +%s)
SANE_DEBUG_EPKOWA=128 scanimage -d "$DEVICE" --x-resolution=$RESOLUTION --y-resolution=$RESOLUTION --format=tiff -x 210 -y 150 > "$OUT/scan.tif" 2>"$OUT/scan.log"
if [ $? -eq 0 ]; then
